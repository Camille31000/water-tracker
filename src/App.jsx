...
export default function App() {
  ...
  const activeDay = editDay || today;
  const activeMl = data.days[activeDay] || 0;
  const activePct = Math.min(activeMl / GOAL_ML, 1);
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        select option { background: #0a1a28; color: #c8e6f5; }
        ...
        {view === "home" && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "8px 16px 16px", gap: 10, zIndex: 1, overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
              <button onClick={() => setEditDay(null)} style={{ background: "none", border: "none", color: "#5eb8e8", fontSize: 13, fontFamily: "'DM Mono'", fontWeight: 600, opacity: editDay ? 0.45 : 1, padding: 0 }}>aujourd'hui</button>
              <span style={{ opacity: 0.3 }}>·</span>
              <select value={editDay || ""} onChange={e => setEditDay(e.target.value || null)} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#c8e6f5", fontSize: 12, padding: "5px 8px", fontFamily: "'DM Mono'", flex: 1 }}>
                <option value="">modifier un jour passé</option>
                {last30.slice(0, -1).reverse().map(d => <option key={d} value={d}>{formatDate(d)}</option>)}
              </select>
            </div>
            {editDay && <p style={{ fontSize: 11, color: "#5eb8e8", fontFamily: "'DM Mono'", flexShrink: 0 }}>✏️ {formatDate(editDay)}</p>}

            {/* Glass */}
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
              <div style={{ width: 120, height: 150, border: "2px solid rgba(94,184,232,0.25)", borderRadius: "12px 12px 20px 20px", overflow: "hidden", position: "relative", background: "rgba(94,184,232,0.04)" }}>
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(180deg, rgba(94,184,232,0.6) 0%, rgba(30,100,180,0.8) 100%)", height: `${activePct * 100}%`, transition: "height 0.5s cubic-bezier(0.34,1.56,0.64,1)", borderRadius: "0 0 18px 18px", ...(ripple ? { animation: "fillRipple 0.5s ease" } : {}) }} />
                <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 2 }}>
                  <span style={{ fontSize: 28, fontWeight: 700, color: "#fff", textShadow: "0 1px 8px rgba(0,0,0,0.4)" }}>{mlToLabel(activeMl)}</span>
                  <span style={{ fontSize: 11, opacity: 0.7, fontFamily: "'DM Mono'" }}>/ 2L</span>
                </div>
              </div>
              {activePct >= 1 && <p style={{ marginTop: 6, fontSize: 12, color: "#5eb8e8" }}>objectif atteint 🎉</p>}
            </div>
            ...
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              {[
                ...
                { ml: -250, label: "−250", sub: "corriger", highlight: false, dim: true },
              ].map(({ ml, label, sub, highlight, dim }) => (
                <button key={label} onClick={() => addMl(activeDay, ml)} style={{ flex: 1, background: highlight ? "rgba(94,184,232,0.18)" : dim ? "rgba(255,255,255,0.04)" : "rgba(94,184,232,0.1)", border: `1px solid ${highlight ? "rgba(94,184,232,0.35)" : dim ? "rgba(255,255,255,0.08)" : "rgba(94,184,232,0.2)"}`, borderRadius: 14, padding: "12px 6px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, color: "#c8e6f5" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 5, flex: 1, minHeight: 0 }}>
                {last7.map(d => (
                  <div key={d.key} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", height: "100%", gap: 3 }}>
                    ...
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: d.met ? "#5eb8e8" : "rgba(255,255,255,0.1)", flexShrink: 0 }} />
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, padding: "12px 10px", flexShrink: 0 }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 3 }}>
                ...
                {monthDays.map(day => {
                  ...
                  return (
                    <div key={day} style={{ borderRadius: 7, padding: "4px 2px", display: "flex", flexDirection: "column", alignItems: "center", gap: 1, minHeight: 36, background: isFuture ? "transparent" : met ? "rgba(94,184,232,0.2)" : hasData ? "rgba(255,255,255,0.04)" : "transparent", border: day === today ? "1px solid rgba(94,184,232,0.5)" : "1px solid transparent", opacity: isFuture ? 0.2 : 1 }}>
                      ...
                      {!isFuture && hasData && <span style={{ fontSize: 8, fontFamily: "'DM Mono'", opacity: 0.6 }}>{mlToLabel(ml)}</span>}
                    </div>
                  );
                })}
              </div>
